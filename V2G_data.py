...
""" Extract Car from Dataset """

#List of all car names to extract:
...
## Insert one car into EV_car
EV_Car = EV_data.loc[EV_data['CarName'] == 'MCV2G03160007']

#Extract data for when the car is providing service and remove error values
df_car = pd.DataFrame(EV_Car)
df_car_avil = df_car.loc[df_car['Available'] == True]
df_car_avil = df_car_avil.loc[df_car_avil['ev_state'] == 'V2G']
df_car_avil = df_car_avil.loc[df_car_avil['EnergyStored'] != 21000]
df_car_avil = df_car_avil.loc[df_car_avil['EnergyStored'] != 0]


enery_stored = df_car_avil.iloc[:,7]
energy_cap = 24000

#Calculate SOC
SOC_car = np.array(enery_stored) / energy_cap


#Time variables
x_tick_car = df_car_avil.iloc[:,0]
time_x_car, date_x_car = Split_Time(x_tick_car)
x = range(len(time_x_car))
...
time_sim, date_sim, xticks_sim = rearrange(x_tick_car)
...
SOC_car = SOC_car[:len(freq_DK2)]

SOC_car_ave = np.average(SOC_car.astype(float))
SOC_car_ave = np.full((len(SOC_car),) , SOC_car_ave)
...
# Pearson correlation and regression errors between full dataset

pearsonr = sc.stats.pearsonr(SOC_car, SOC_N)
y_true = SOC_car
y_pred = SOC_N
...
#### Plot full SOC of car and corresponding simulated SOC
...
ax.plot(x_1 ,  SOC_car, label = 'Car 2 - MCV2G03160012' , color =  'navy')
ax.plot(x_1, SOC_N, label = 'FCR - Simulated', color = 'forestgreen')
ax.set_xlabel('Time', fontsize = 13)
ax.set_ylabel('SOC [p.u.]', fontsize = 13)
ax.set_ylim([0,1])
plt.xticks(x[::(21600*4)], date_x_car[::(21600*4)], rotation = 'vertical')
box = ax.get_position()
...
# Extract certain time periods from data
SOC_car_span = SOC_car[(16*24*3600):(21*24*3600)]
SOC_N_span = SOC_N[(16*24*3600):(21*24*3600)]


SOC_car_span1 = SOC_car[(7*24*3600 + 35000 + 10800 *3):(10*24*3600 + 10800)]
...
plt.xticks(x_1[(7*24*3600 + 35000 + 10800 *3):(10*24*3600 + 10800)][::(10800)], 
               time_x_car[(7*24*3600 + 35000 + 10800 *3):(10*24*3600 + 10800)][::(10800)], rotation = 'vertical')
ax.legend(loc=3)




## Vector of average SOC in order to provide refrence for RMSE
